...
class _MessageListScreenState extends State<MessageListScreen>{
    ...
    return Scaffold(
      ...
      body: StreamBuilder<List<MessageModel>>(
        ...
        builder: (context, asyncSnapshot){
          if(!asyncSnapshot.hasData){ //데이터가 없을 경우 로딩위젯 표시
            ...
            List<MessageModel> messages = asyncSnapshot.data!; //비동기 데이터 존재시 리스트뷰 표시
            return Column(
  ...
  Stream<List<MessageModel>> streamMessages(){
    try{
      //찾으려는 컬렉션의 스냅샷(Stream)을 가져온다.
      final Stream<QuerySnapshot> snapshots = FirebaseFirestore.instance.collection('chatrooms/wuCOBDjEjmaPUkW13rso/messages').orderBy('sendDate').snapshots();
  ...
  void _onPressedSendButton(){
    try{
      ...
      firestore.collection('chatrooms/wuCOBDjEjmaPUkW13rso/messages').add(messageModel.toMap());
